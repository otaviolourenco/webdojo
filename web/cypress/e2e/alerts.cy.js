describe('Validações de alertas em JavaScript', () => {
  ...
  it('Deve interagir com um prompt, inserir texto e validar uma mensagem', () => {
    cy.window().then((win) => {
      cy.stub(win, 'prompt').returns('Lourenço');
    });

    cy.on('window:alert', (msg) => {
      expect(msg).to.equal('Olá Lourenço! Boas-vindas ao WebDojo!');
    });
